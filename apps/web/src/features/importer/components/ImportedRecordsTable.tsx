...
export function ImportedRecordsTable({ records }: { records: ImportedRecord[] }) {
  if (records.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-[#D1D5DB] bg-white px-4 py-10 text-center">
        <p className="text-sm font-semibold text-[#111827]">No imported records yet</p>
          ...
          <thead className="sticky top-0 z-10 bg-[#F9FAFB] text-xs uppercase tracking-wide text-[#111827]">
            <tr>
              ...
              {CRM_FIELDS.map((field) => (
                <th className="border-b border-[#E5E7EB] px-4 py-3 font-bold" key={field}>
                  {field}
                </th>
              ))}
            </tr>
                ...
                {CRM_FIELDS.map((field) => (
                  ...
                  >
                    {field === "crm_status" ? (
                      ...
                      <span className="line-clamp-2 break-words">{record[field] || "-"}</span>
                    )}
                  </td>
                ))}
              </tr>
