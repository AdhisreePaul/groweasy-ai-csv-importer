...
export function SkippedRecordsTable({ records }: { records: SkippedRecord[] }) {
  if (records.length === 0) {
    return (
      ...
      <div className="max-h-80 overflow-auto">
        <table className="min-w-[680px] w-full border-separate border-spacing-0 text-left text-sm">
          <thead className="sticky top-0 z-10 bg-white text-xs uppercase tracking-wide text-[#111827]">
            <tr>
              <th className="border-b border-[#E5E7EB] px-4 py-3 font-bold">Row</th>
              <th className="border-b border-[#E5E7EB] px-4 py-3 font-bold">Reason</th>
              <th className="border-b border-[#E5E7EB] px-4 py-3 font-bold">Raw Data</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#E5E7EB]">
            {records.map((record) => (
              <tr className="text-[#374151]" key={record.source_row}>
                ...
                <td className="px-4 py-4 font-medium text-amber-800">{record.reason}</td>
