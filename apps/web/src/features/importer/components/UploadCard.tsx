...
const dropzoneBaseClassName = [
  "flex min-h-72 cursor-pointer flex-col items-center justify-center rounded-lg",
  "border border-dashed px-5 py-8 text-center transition focus-within:outline-none",
  "focus-within:ring-2 focus-within:ring-leaf focus-within:ring-offset-2"
].join(" ");

const chooseCsvClassName = [
  "mt-4 inline-flex h-10 items-center justify-center gap-2 rounded-md bg-leaf px-4",
...
const removeButtonClassName = [
  ...
  "border-line bg-white text-muted transition hover:text-rose focus:outline-none",
  "focus:ring-2 focus:ring-rose focus:ring-offset-2"
].join(" ");

const confirmButtonClassName = [
  "mt-4 inline-flex h-10 w-full items-center justify-center gap-2 rounded-md",
  "bg-leaf px-4 text-sm font-semibold text-white shadow-sm transition",
  "hover:bg-leaf-dark focus:outline-none focus:ring-2 focus:ring-leaf focus:ring-offset-2",
  "disabled:cursor-not-allowed disabled:border disabled:border-line disabled:bg-white",
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-muted">
            Start with a raw preview. The backend and AI extraction stay idle until the final
            confirmation step.
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
        <label
          className={`${dropzoneBaseClassName} ${
            isDragging
        ...
        >
          ...
          <span className="mt-2 max-w-lg text-sm leading-6 text-muted">
            Any valid CSV format is accepted, including messy lead sheets from ad campaigns,
            WhatsApp follow-ups, telephony exports, and manual CRM lists.
          </span>
          <span className={chooseCsvClassName}>
        ...
        <div className="rounded-lg border border-line bg-white p-4">
          ...
          {selectedFile ? (
            <div className="mt-4 rounded-md border border-line bg-soft p-3">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs text-muted">{formatFileSize(selectedFile.size)}</p>
                </div>
                <button
            ...
            <p className="mt-4 rounded-md border border-dashed border-line bg-soft p-3 text-sm leading-6 text-muted">
              No CSV selected yet. Drag a file into the upload area or use the file picker.
            </p>
          )}
          ...
          <button
            ...
            type="button"
          >
            {isParsing ? "Reading CSV" : isImporting ? "Importing leads" : "Confirm import"}
